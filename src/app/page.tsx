...
export default function Page() {
  ...
  const [openCats, setOpenCats] = useState<Record<string, boolean>>({ ent: true, hyg: true, kitch: true, cloth: true })
  ...
  const card: React.CSSProperties = { background: '#fff', borderRadius: 20, padding: 20, marginBottom: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }
  const filterBtn = (active: boolean): React.CSSProperties => ({ flexShrink: 0, padding: '6px 14px', borderRadius: 100, border: `1.5px solid ${active ? '#6C63FF' : '#e5e2dd'}`, background: active ? '#6C63FF' : '#fff', color: active ? '#fff' : '#666', fontSize: 13, fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'inherit' })
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px 60px' }}>
      ...
      <div style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 22, fontWeight: 700, color: '#1a1a1a' }}>{done} <span style={{ fontSize: 13, color: '#888', fontWeight: 400 }}>/ {total}</span></span>
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <button onClick={exportJSON} style={{ flex: 1, padding: '10px 0', borderRadius: 14, border: '1.5px solid #e5e2dd', background: '#fff', fontSize: 13, color: '#555', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>⬇ エクスポート</button>
        <button onClick={() => fileRef.current?.click()} style={{ flex: 1, padding: '10px 0', borderRadius: 14, border: '1.5px solid #e5e2dd', background: '#fff', fontSize: 13, color: '#555', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 500 }}>⬆ インポート</button>
      ...
      {CATS.filter(c => filter === 'all' || filter === c.id || filter === 'undone' || filter === 'done').map(cat => {
        ...
        return (
          <div key={cat.id} style={{ ...card, marginBottom: 12, padding: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', cursor: 'pointer' }} onClick={() => setOpenCats(p => ({ ...p, [cat.id]: !p[cat.id] }))}>
            ...
            {isOpen && (
              <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: 4 }}>
                {items.length === 0
                  ...
                  : items.map(item => (
                    <div key={item.id} onClick={() => toggle(cat.id, item.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', background: '#fafaf8', borderRadius: 12, cursor: 'pointer', opacity: item.checked ? 0.55 : 1 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
        <select value={addCat} onChange={e => setAddCat(e.target.value)} style={{ padding: '12px 10px', borderRadius: 14, border: '1.5px solid #e5e2dd', background: '#fff', fontSize: 13, fontFamily: 'inherit', color: '#555', outline: 'none' }}>
          {CATS.map(c => <option key={c.id} value={c.id}>{c.icon} {c.name}</option>)}
        </select>
        <input value={addLabel} onChange={e => setAddLabel(e.target.value)} onKeyDown={e => e.key === 'Enter' && addItem()} placeholder="アイテムを追加…" style={{ flex: 1, padding: '12px 16px', borderRadius: 14, border: '1.5px solid #e5e2dd', background: '#fff', fontSize: 14, fontFamily: 'inherit', color: '#1a1a1a', outline: 'none' }} />
